...
import 'package:lgs_audiopedia/data/pashto_data/dengue_data_pashto.dart';
...
class TopicsOfDenguePashto extends StatefulWidget {
  const TopicsOfDenguePashto({Key? key}) : super(key: key);

  @override
  State<TopicsOfDenguePashto> createState() => _TopicsOfDenguePashto();
}

class _TopicsOfDenguePashto extends State<TopicsOfDenguePashto> {
  List<Topic>? topics;
  String query = '';

  List<Topic> topicData = List.generate(
    dengueDataPashto.length,
    (index) => Topic(
        id: dengueDataPashto[index].id,
        title: dengueDataPashto[index].title,
        description: dengueDataPashto[index].description),
  );
  @override
  void initState() {
    super.initState();
    // topics = allTopics;
    topicData = dengueDataPashto;
  }

  void searchBook(String query) {
    final books = dengueDataPashto.where((book) {
